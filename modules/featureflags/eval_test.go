...
func TestComputeEffectivePrecedence(t *testing.T) {
	...
	if !out["f"] {
		t.Fatalf("global true should enable")
	}
	...
	if out["f"] {
		t.Fatalf("tenant false should override global true")
	}
	...
	if !out["f"] {
		t.Fatalf("any=true for roles should enable when any true")
	}
	...
	if out["f"] {
		t.Fatalf("user false should override others")
	}
}
